...
class Data(object):
    def __init__(self, folder, seed, type_check, domain_size, no_extra_facts):
        ...
        self.relation_file = os.path.join(folder, "relations_ext.txt")
        ...
        self.train_file_soft = os.path.join(folder, "train_triple_scores.txt")
        self.test_file_soft = os.path.join(folder, "test_triple_scores.txt")
        ...
        self.train_soft, self.num_train_soft = self._parse_triplets_soft(self.train_file_soft, self.train)
        self.test_soft, self.num_test_soft = self._parse_triplets_soft(self.test_file_soft, self.train)

        self.matrix_db_train = self._db_to_matrix_db(self.train)
        self.matrix_db_train_soft = self._db_to_matrix_db_soft(self.train_soft, [])
        matrix_db_valid = self._db_to_matrix_db(self.valid)
        self.matrix_db_valid = self._combine_two_mdbs(self.matrix_db_train, self.matrix_db_train_soft)
        # self.matrix_db_valid = self._combine_two_mdbs(self.matrix_db_train, self.matrix_db_valid)
        self.matrix_db_test = self._combine_two_mdbs(self.matrix_db_train, matrix_db_valid)
        self.matrix_db_test = self._combine_two_mdbs(self.matrix_db_test, self.matrix_db_train_soft)
        # if self.share_db:
        ...
        #         = self._parse_triplets(self.train_file)
        #     self.valid_facts, self.num_valid_fact \
        #         = self._parse_triplets(self.valid_file)
        #     self.test_facts, self.num_test_fact \
        #         = self._parse_triplets(self.test_file)
        #     self.matrix_db_train = self._db_to_matrix_db(self.train_facts)
        #     self.matrix_db_valid = self._db_to_matrix_db(self.valid_facts)
        #     self.matrix_db_valid = self._combine_two_mdbs(self.matrix_db_train, self.matrix_db_valid)
        #     self.matrix_db_test = self.matrix_db_valid
        ...
        self.query_for_rules = list(set(list(zip(*self.train))[0]) | set(list(zip(*self.test))[0]) | set(
            list(zip(*self._augment_with_reverse(self.train)))[0]) | set(
            list(zip(*self._augment_with_reverse(self.test)))[0]))
    ...
    def _parse_triplets_soft(self, file, db):
        """Convert (head, relation, tail) to (relation, head, tail)"""
        facts = set()
        for i, fact in enumerate(db):
            rel = fact[0] + int(self.num_relation / 2)
            head = fact[1]
            tail = fact[2]
            facts.add('{}||{}||{}'.format(head, rel, tail))
        output = []
        with open(file) as f:
            for line in f:
                l = line.strip().split("\t")
                assert (len(l) == 3)
                h, r, t = l[0].replace(' ', '_').split('||')
                r = r + '_ext'
                score = float(l[1])
                if score < 0.5: continue
                # fact = '{}||{}||{}'.format(self.entity_to_number[h], self.relation_to_number[r], self.entity_to_number[t])
                # if fact in facts: continue
                # fact = '{}||{}||{}'.format(self.entity_to_number[t], self.relation_to_number[r], self.entity_to_number[h])
                # if fact in facts: continue
                output.append((self.relation_to_number[r],
                               self.entity_to_number[h],
                               self.entity_to_number[t],
                               score))
        print('Soft triple size: {}'.format(len(output)))
        return output, len(output)
    ...
    def _db_to_matrix_db_soft(self, db, ban_facts):
        matrix_db = {r: ([[0, 0]], [0.], [self.num_entity, self.num_entity])
                     for r in range(self.num_relation)}
        facts = set()
        for fact in ban_facts:
            r = fact[0] + int(self.num_relation / 2)
            fact_ = '{}\t{}\t{}'.format(r, fact[1], fact[2])
            facts.add(fact_)
        for i, fact in enumerate(db):
            rel = fact[0]
            head = fact[1]
            tail = fact[2]
            # triple = '{}\t{}\t{}'.format(fact[0], fact[1], fact[2])
            # if triple in facts: continue
            # triple = '{}\t{}\t{}'.format(fact[0], fact[2], fact[1])
            # if triple in facts: continue
            value = fact[3]
            # value = 1
            matrix_db[rel][0].append([head, tail])
    ...
    def reset(self, batch_size):
        ...
        if not self.type_check:
            ...
            self.num_batch_valid = self.num_valid
            self.num_batch_test = self.num_test
        else:
    ...
    def _next_batch_test(self, start, size, samples):
        assert (start < size)
    ...
    def next_test(self):
        self.test_start, this_batch, this_batch_id = self._next_batch_test(self.test_start,
                                                                           self.num_test,
                                                                           self.test)
        ...
        extra_facts = [fact for i, fact in enumerate(self.test) if i not in this_batch_id]
        extra_mdb = self._db_to_matrix_db(extra_facts)
        extra_mdb_soft = self._db_to_matrix_db_soft(self.test_soft, this_batch)
        augmented_mdb = self._combine_two_mdbs(extra_mdb, self.matrix_db_test)
        augmented_mdb = self._combine_two_mdbs(augmented_mdb, extra_mdb_soft)
    ...
    def next_valid(self):
        self.valid_start, this_batch, this_batch_id = self._next_batch_test(self.valid_start,
                                                                            self.num_valid,
                                                                            self.valid)
        # if self.share_db and self.use_extra_facts:
        ...
        extra_facts = [fact for i, fact in enumerate(self.valid) if i not in this_batch_id]
        extra_mdb = self._db_to_matrix_db(extra_facts)
        extra_mdb_soft = self._db_to_matrix_db_soft(self.test_soft, this_batch)
        augmented_mdb = self._combine_two_mdbs(extra_mdb, self.matrix_db_valid)
        augmented_mdb = self._combine_two_mdbs(augmented_mdb, extra_mdb_soft)
        matrix_db = augmented_mdb
        if self.type_check:
            query = this_batch[0][0]
    ...
    def next_train(self):
        ...
        extra_facts = [fact for i, fact in enumerate(self.train) if i not in this_batch_id]
        extra_mdb = self._db_to_matrix_db(extra_facts)
        extra_mdb_soft = self._db_to_matrix_db_soft(self.train_soft, this_batch)
        augmented_mdb = self._combine_two_mdbs(extra_mdb, extra_mdb_soft)
        matrix_db = augmented_mdb
        if self.type_check:
            query = this_batch[0][0]
