...
class Experiment():
    ...
    def one_epoch_train(self):
        ...
        print('num_batch_train', self.data.num_batch_train)
    ...
    def one_epoch_valid(self):
        ...
        print('num_batch_valid', self.data.num_batch_valid)
    ...
    def one_epoch_test(self):
        loss, in_top = self.one_epoch("test", 
                                      ...
                                      self.data.next_test)
        print('num_batch_test', self.data.num_batch_test)
    ...
    def get_predictions(self):
        ...
        all_in_top = []
        mrr, hit_1, hit_3, hit_10, count = 0, 0, 0, 0, 0
        for batch in tqdm(range(int(self.data.num_batch_test))):
            ...
            for i, (q, h, t) in enumerate(zip(qq, hh, tt)):
                ...
                eql_fn = lambda t: (t[1] == p_head)
                this_predictions = filter(eval_fn, enumerate(predictions_this_batch[i, :]))
                eql_predictions = filter(eql_fn, enumerate(predictions_this_batch[i, :]))
                this_predictions = [self.data.number_to_entity[j] for j, _ in this_predictions]
                eql_predictions = [self.data.number_to_entity[j] for j, _ in eql_predictions]
                m = len(this_predictions)
                n = len(eql_predictions)
                rank = m + (n + 1) / 2
                mrr += 1 / rank
                if rank == 1: hit_1 += 1
                if rank <= 3: hit_3 += 1
                ...
                if self.option.query_is_language:
                    ...
                    # this_predictions.extend(eql_predictions)
                    # this_predictions.append((h, p_head))
                    # print(len(this_predictions_new))
                    # this_predictions = [str(j) for j, _ in this_predictions]

                    q_string = self.data.parser["query"][q]
                    h_string = self.data.number_to_entity[h]
                    t_string = self.data.number_to_entity[t]
                    to_write = [q_string, h_string, t_string] + this_predictions + ['#'] + eql_predictions + [self.data.number_to_entity[h]]
        ...
        print(msg)
        self.log_file.write(msg + "\n")
        mrr, hit_1, hit_3, hit_10 = mrr / count, hit_1 / count,  hit_3 / count, hit_10 / count
        print("Eval result: Mrr: {}, Hit@1: {}, Hit@3: {}, Hit@10: {}, Count: {}".format(mrr, hit_1, hit_3, hit_10, count))
    ...
    def get_attentions(self):
        ...
        for queries in query_batches:
            ...
            print(len(attention_operators), len(attention_operators[0]), len(attention_operators[0][0]), attention_operators[0][0][0].shape, len(queries))
            print(len(attention_memories), attention_memories[0].shape, attention_memories[0][0].shape, attention_memories[0][0][0].shape)
            ...
            for i in range(len(queries)):
                all_attention_operators[queries[i]] \
                    = [[attn[i]
                       ...
                       for attn_step in attention_operators[0]]
    ...
    def get_rules(self):
        ...
        with open(os.path.join(self.option.this_expsdir, "rules_0.txt"), "w") as f:
