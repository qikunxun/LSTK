...
class Learner(object):
    ...
    def _build_graph(self):
        ...
        for i_rank in range(self.rank):
            for t in range(self.num_step):
                if t < self.num_step - 1 and i_rank == 0:
                    self.attention_memories_list.append(
                        tf.nn.softmax(
                            tf.squeeze(
                                tf.matmul(
                                    tf.expand_dims(self.rnn_outputs_list[i_rank][t], 1),
                                    tf.stack(self.rnn_outputs_list[i_rank][0:t + 1], axis=2)),
                                squeeze_dims=[1])))
                    ...
                    for r in range(int(self.num_operator/2)):
                        for op_matrix, op_attn in zip(
                                        ...
                                        [self.attention_operators_list[i_rank][t][r], 
                                         self.attention_operators_list[i_rank][t][r+int(self.num_operator/2)]]):
        ...
        if not self.accuracy:
            ...
            #                 targets=self.heads,
            #                 k=self.top_k)
            truth_scores = tf.reduce_sum(self.predictions * tf.one_hot(self.heads, self.predictions.shape[-1]), axis=-1)
            m = tf.reduce_sum(tf.cast(tf.greater(self.predictions, tf.expand_dims(truth_scores, axis=-1)), dtype=tf.float32), axis=-1)
            n = tf.reduce_sum(tf.cast(tf.equal(self.predictions, tf.expand_dims(truth_scores, axis=-1)), dtype=tf.float32), axis=-1)

            rank = m + (n + 1) / 2
            self.in_top = tf.less_equal(rank, self.top_k)
        else: 
        ...
        self.optimizer_step = self.optimizer.minimize(tf.reduce_mean(self.final_loss))
    ...
    def get_attentions_given_queries(self, sess, queries):
        ...
        mdb = {r: ([(0,0)], [0.], (self.num_entity, self.num_entity)) 
                for r in range(int(self.num_operator / 2))}
        to_fetch = [self.attention_operators_list, self.attention_memories_list]
        fetched = self._run_graph(sess, qq, hh, tt, mdb, to_fetch)
        return fetched[0], fetched[1]
