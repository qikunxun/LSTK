...
KnowledgeGraph::~KnowledgeGraph()
{
    ...
    delete [] e2rn;
    for (int k = 0; k != entity_size; k++) e2rn_valid[k].clear();
    delete [] e2rn_valid;
    for (int k = 0; k != entity_size; k++) e2rn_test[k].clear();
    delete [] e2rn_test;
}
...
void KnowledgeGraph::read_data(char *data_path)
{
    ...
    FILE *fi;
    int bg_size_train, bg_size_valid, bg_size_test;
    ...
    e2rn_valid = new std::vector<std::pair<int, int> > [entity_size];
    for (int k = 0; k != train_triplet_size; k++)
    {
        h = train_triplets[k].h; r = train_triplets[k].r; t = train_triplets[k].t;
        e2rn_valid[h].push_back(std::make_pair(r, t));
        bg_size_valid += 1;
    }
    e2rn_test = new std::vector<std::pair<int, int> > [entity_size];
    for (int k = 0; k != train_triplet_size; k++)
    {
        h = train_triplets[k].h; r = train_triplets[k].r; t = train_triplets[k].t;
        e2rn_test[h].push_back(std::make_pair(r, t));
        bg_size_test += 1;
    }
    strcpy(s_file, data_path);
    strcat(s_file, "/train_ext.txt");
    fi = fopen(s_file, "rb");
    if (fi == NULL)
    {
    ...
    while (1)
    {
        ...
        triplet.h = h; triplet.t = t; triplet.r = r;
        e2rn[h].push_back(std::make_pair(r, t));
        e2rn[t].push_back(std::make_pair(r + relation_size, h));
        e2rn_valid[h].push_back(std::make_pair(r, t));
        e2rn_valid[t].push_back(std::make_pair(r + relation_size, h));
        e2rn_test[h].push_back(std::make_pair(r, t));
        e2rn_test[t].push_back(std::make_pair(r + relation_size, h));
        bg_size_train += 2;
        bg_size_valid += 2;
        bg_size_test += 2;
    }
    ...
    for (int k = 0; k != valid_triplet_size; k++)
    {
        h = valid_triplets[k].h; r = valid_triplets[k].r; t = valid_triplets[k].t;
        e2rn_valid[h].push_back(std::make_pair(r, t));
        bg_size_valid += 1;
    }
    for (int k = 0; k != valid_triplet_size; k++)
    ...
    test_triplet_size = int(test_triplets.size());
        for (int k = 0; k != test_triplet_size; k++)
    {
        h = test_triplets[k].h; r = test_triplets[k].r; t = test_triplets[k].t;
        e2rn_test[h].push_back(std::make_pair(r, t));
        bg_size_test += 1;
    }

    strcpy(s_file, data_path);
    strcat(s_file, "/test_ext.txt");
    fi = fopen(s_file, "rb");
    if (fi == NULL)
    {
        printf("ERROR: file of test triplets not found!\n");
    ...
    int score_size;
    strcat(s_file, "/scores.txt");
    fi = fopen(s_file, "rb");
    if (fi == NULL)
    {
        printf("ERROR: file of triplet scores not found!\n");
        exit(1);
    }
    while (1)
    {
        if (fscanf(fi, "%s %s %s %f", s_head, s_rel, s_tail, &s_score) != 4) break;
        //printf("data: %s %s %s %f          \n", s_head, s_rel, s_tail, s_score);
        if (ent2id.count(s_head) == 0 || ent2id.count(s_tail) == 0 || rel2id.count(s_rel) == 0) continue;

        h = ent2id[s_head]; t = ent2id[s_tail]; r = rel2id[s_rel];
        all_scores[h][r][t] = s_score;
        all_scores[t][r + relation_size][h] = s_score;
        score_size += 2;
    }
    fclose(fi);
    ...
    printf("#Train graph size: %d          \n", bg_size_train);
    printf("#Valid graph size: %d          \n", bg_size_valid);
...
void KnowledgeGraph::rule_destination(int e, Rule rule, std::vector<std::pair<int, float> > &dests, Triplet removed_triplet, int flag)
{
    std::queue< std::pair<int, int> > queue;
    queue.push(std::make_pair(e, 0));
    std::queue< std::pair<int, float> > queue_score;
    queue_score.push(std::make_pair(e, 1.));
    int current_e, current_d, current_r, next_e;
    float current_score;
    std::vector<std::pair<int, int> > *e2rn_cur;
    if (flag == 0){
        e2rn_cur = e2rn;
    }
    if (flag == 1){
    ...
    while (!queue.empty())
    {
        ...
        queue.pop();
        std::pair<int, float> pair_score = queue_score.front();
        current_score = pair_score.second;
        queue_score.pop();
        if (current_d == int(rule.r_body.size()))
        {
            dests.push_back(std::make_pair(current_e, current_score));
...
//            }
        ...
        for (int k = 0; k != int(e2rn_cur[current_e].size()); k++)
        {
            if (e2rn_cur[current_e][k].first != current_r) continue;
            next_e = e2rn_cur[current_e][k].second;
            if (current_e == removed_triplet.h && current_r == removed_triplet.r && next_e == removed_triplet.t) continue;
            int ext_r = -1;
            if (removed_triplet.r >= relation_size / 2) {
                ext_r = removed_triplet.r - relation_size / 4;
            } else{
                ext_r = removed_triplet.r + relation_size / 4;
            }
            if (current_e == removed_triplet.h && current_r == ext_r && next_e == removed_triplet.t) continue;

            queue.push(std::make_pair(next_e, current_d + 1));
            queue_score.push(std::make_pair(next_e, current_score * all_scores[current_e][current_r][next_e]));
        }
    }
}
...
void ReasoningPredictor::learn_thread(int thread)
{
    ...
    std::vector<std::pair<int, float> > dests;
    Triplet triplet;
    ...
    for (int T = bg; T != ed; T++)
    {
        ...
        {
            dests.clear();
            p_kg->rule_destination(h, rel2rules[r][index], dests, p_kg->train_triplets[T], 0);
            
            for (int i = 0; i != int(dests.size()); i++)
            {
                dest = dests[i].first;
                score = dests[i].second;
                if (dest2logit.count(dest) == 0) dest2logit[dest] = 0;
                if (dest2index.count(dest) == 0) dest2index[dest] = std::vector<int>();
                dest2logit[dest] += rel2rules[r][index].wt.data * score / temperature;
...
void ReasoningPredictor::H_score_thread(int thread)
{
    ...
    RankListEntry entry;
    std::map<int, std::vector<int> > dest2index;
    std::map<int, std::vector<std::pair<int, float> > > index2dest;
    std::map<int, std::vector<std::pair<int, float> > >::iterator iter;
    ...
    for (int T = bg; T != ed; T++)
    {
        ...
        for (index = 0; index != int(rel2rules[r].size()); index++)
        {
            ...
            for (int i = 0; i != int(dests.size()); i++)
            {
                ...
                score = dests[i].second;
                if (dest2index.count(dest) == 0) dest2index[dest] = std::vector<int>();
                if (index2dest.count(index) == 0) index2dest[index] = std::vector<std::pair<int, float> >();
                dest2index[dest].push_back(index);
                index2dest[index].push_back(std::make_pair(dest, score));
            }
        }
        ...
        for (iter = index2dest.begin(); iter != index2dest.end(); iter++)
        {
            ...
            for (int i = 0; i != int(dests.size()); i++)
            {
                dest = dests[i].first;
                score = dests[i].second;
                if (dest == t) entry.val += rel2rules[r][index].wt.data * score;
                entry.val -= rel2rules[r][index].wt.data * score / dest2index.size();
            }
...
void ReasoningPredictor::evaluate_thread(int thread)
{
    ...
    for (int T = bg; T != ed; T++)
    {
        ...
        {
            dests.clear();
            p_kg->rule_destination(h, rel2rules[r][index], dests, (*p_triplets)[T], flag);
            
            for (int i = 0; i != int(dests.size()); i++)
            {
                dest = dests[i].first;
                score = dests[i].second;
                rank_list[dest].val += rel2rules[r][index].wt.data * score;
            }
        }
