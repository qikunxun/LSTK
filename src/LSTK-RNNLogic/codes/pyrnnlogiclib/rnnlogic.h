...
protected:
    ...
    std::vector<std::pair<int, int> > *e2rn, *e2rn_valid, *e2rn_test;
    std::map<int, std::map<int, std::map<int, float> > > all_scores;
...
public:
    ...
    void rule_destination(int e, Rule rule, std::vector<std::pair<int, float> > &dests, Triplet removed_triplet, int flag);
};
