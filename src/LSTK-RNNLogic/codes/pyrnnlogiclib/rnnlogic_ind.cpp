...
//
//        h = ent2id[s_head]; t = ent2id[s_tail]; r = rel2id[s_rel];
//        std::string triple = std::to_string(h) + "\t" + std::to_string(r) + "\t" + std::to_string(t);
//        all_scores[triple] = s_score;
//        std::string triple_inv = std::to_string(t) + "\t" + std::to_string((r + relation_size)) + "\t" + std::to_string(h);
//        all_scores[triple_inv] = s_score;
//    }
//    fclose(fi);
...
void KnowledgeGraph::rule_destination(int e, Rule rule, std::vector<std::pair<int, float> > &dests, Triplet removed_triplet, int flag)
{
    ...
    while (!queue.empty())
    {
        ...
        for (int k = 0; k != int(e2rn_cur[current_e].size()); k++)
        {
            ...
            std::string triple = std::to_string(current_e) + "\t" + std::to_string(current_r) + "\t" + std::to_string(next_e);
            queue_score.push(std::make_pair(next_e, current_score));
        }
    }
}
